...
if [ ! -n "${JAVA_OPTS}" ]; then
    echo "demo-service使用默认JAVA_OPTS与disconf信息:disconf.conf_server_host=192.168.0.40"
	JAVA_OPTS="-Ddisconf.enable.remote.conf=true -Ddisconf.app=uyun -Ddisconf.env=local -Ddisconf.version=2_0_0 -Ddisconf.conf_server_host=192.168.0.40/disconf"
fi
...
APP_NAME=dlbanktar-service

function startDemoService()
{
    ...
    cd ${UP_DIR}/lib
    echo ----------------
    echo java -Dfile.encoding=UTF-8  ${RUN_OPTS} ${JAVA_OPTS} -cp ${CLASSPATH} -jar dlbanktar-service-0.0.1-SNAPSHOT.jar
    echo -----------------
	nohup java -Dfile.encoding=UTF-8  ${RUN_OPTS} ${JAVA_OPTS} -cp ${CLASSPATH} -jar dlbanktar-service-0.0.1-SNAPSHOT.jar > /dev/null 2>&1 &
	out $APP_NAME
}
