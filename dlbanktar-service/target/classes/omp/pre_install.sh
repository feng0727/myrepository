...
logfile="dlbanktar-service-install.log"
...
DISCONF_USER=admin
DISCONF_PASS=admin
DISCONF_UYUN_dlbanktar_KEY=dlbanktar.properties
...
#修改企业版参数
sh -c "sed -i 's@bat.developer.mode=true@@' ./dlbanktar.properties"
...
# 获取configId配置项
curl -b cookies -c cookies -sS "$CONF_LIST_URL?appId=$UYUN_APP_ID&envId=$ENV_ID&version=$DISCONF_VERSION" | grep -v err_no > result
demo_CONFIG_ID=$(cat result | ./jq '.page.result[] | select(.key == "dlbanktar.properties") | .configId')

if [ -n "$demo_CONFIG_ID" ]; then
    echo "dlbanktar.properties is exist. "
else
	demo_UPLOAD_URL="$FILE_UPLOAD_URL?appId=$UYUN_APP_ID&envId=$ENV_ID&version=$DISCONF_VERSION"
	
	uploadFile "$demo_CONFIG_ID" "dlbanktar.properties" "$demo_UPLOAD_URL"
